...
describe MagicSeven do
  ...
  let(:game_with_fixed_number_choice) { MagicSeven.new(8) }
  describe '#subtract_random_number' do
    context 'when the previous step is 15 and chosen number is 8' do
      it 'returns 7' do
        previous_step = 15
        result = game_with_fixed_number_choice.subtract_random_number(previous_step)
  ...
  # integer as the random_number. Update the context with the number.
    let(:game_with_23_as_choice) { MagicSeven.new(23) }
    describe '#play' do
    ...
    let(:game_with_53523_as_choice) { MagicSeven.new(53523) }
    context 'when the random number is 53523' do
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_with_53523_as_choice.play
        expect(result).to eq(7)
      end
    end

    let(:game_with_big_negative_float_as_choice) { MagicSeven.new(-32462144.124) }
    context 'when the random number is -32462144.124' do
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_with_big_negative_float_as_choice.play
        expect(result).to eq(7)
      end
    end
  end
